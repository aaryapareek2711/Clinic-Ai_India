...
import { localDateInputMin } from '../lib/appointmentSlots'
...
export default function VisitDetailPage() {
  ...
  const [vitalsFormGenerating, setVitalsFormGenerating] = useState(false)
  const vitalsGenerateInFlightRef = useRef(false)
  ...
  useEffect(() => {
    ...
    setVitalsFormGenerating(false)
    vitalsGenerateInFlightRef.current = false
  ...
  const handleGenerateVitalsForm = useCallback(async () => {
    ...
    if (vitalsLocked || vitalsFormVisible || vitalsGenerateInFlightRef.current) return
    vitalsGenerateInFlightRef.current = true
    setVitalsFormGenerating(true)
    try {
                  ...
                  {!isTranscriptionCurrentlyProcessing && !vitalsFormVisible && (
                    <div className="flex flex-wrap items-center gap-3">
                      <button
                        ...
                        type="button"
                      >
                        {vitalsFormGenerating ? 'Generating…' : 'Generate Vitals Form'}
                      </button>
                    </div>
                  )}
